...
func (c *UnidlingController) handleRequest(info types.NamespacedName, lastFired time.Time) (bool, error) {
	...
	deleteIdledAtAnnotation := func(_ unidlingapi.CrossGroupObjectReference, annotations map[string]string) {
		delete(annotations, unidlingapi.IdledAtAnnotation)
	}

	scaleGetter := c.scaleNamespacer.Scales(info.Namespace)
	dcGetter := c.dcNamespacer.DeploymentConfigs(info.Namespace)
	rcGetter := c.rcNamespacer.ReplicationControllers(info.Namespace)
	scaleAnnotater := unidlingutil.NewScaleAnnotater(scaleGetter, dcGetter, rcGetter, deleteIdledAtAnnotation)

	for _, scalableRef := range targetScalables {
		var scale *kextapi.Scale
		var obj runtime.Object

		obj, scale, err = scaleAnnotater.GetObjectWithScale(scalableRef.CrossGroupObjectReference)
		if err != nil {
		...
		if err = scaleAnnotater.UpdateObjectScale(scalableRef.CrossGroupObjectReference, obj, scale); err != nil {
			if errors.IsNotFound(err) {
				utilruntime.HandleError(fmt.Errorf("%s %q does not exist, removing from list of scalables while unidling service %s/%s: %v", scalableRef.Kind, scalableRef.Name, info.Namespace, info.Name, err))
				delete(targetScalablesSet, scalableRef)
			} else {
