...
func NewUnidlingController(scaleNS kextclient.ScalesGetter, endptsNS kclient.EndpointsGetter, evtNS kclient.EventsGetter, resyncPeriod time.Duration) *UnidlingController {
	fieldSet := fields.Set{}
...
func (c *UnidlingController) handleRequest(info types.NamespacedName, lastFired time.Time) (bool, error) {
	...
	scales := c.scaleNamespacer.Scales(info.Namespace)
