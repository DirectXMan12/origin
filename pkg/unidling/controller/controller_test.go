...
import (
	...
	kapireg "k8s.io/kubernetes/pkg/apimachinery/registered"
...
func prepFakeClient(t *testing.T, nowTime time.Time, scales ...kextapi.Scale) (*kfake.Clientset, *fakeResults) {
	...
	mapper := kapireg.RESTMapper()
	fakeClient.PrependReactor("get", "*", func(action ktestingcore.Action) (bool, runtime.Object, error) {
		if action.GetSubresource() != "scale" {
			return false, nil, nil
		}

		var kind string
		if action.GetResource().Resource == "DeploymentConfig" {
		...
		for _, scale := range scales {
			if kind == scale.Kind && action.(ktestingcore.GetAction).GetName() == scale.Name {
	...
	fakeClient.PrependReactor("update", "*", func(action ktestingcore.Action) (bool, runtime.Object, error) {
		if action.GetSubresource() != "scale" {
			return false, nil, nil
		}
		...
		obj := action.(ktestingcore.UpdateAction).GetObject().(*kextapi.Scale)

		for _, scale := range scales {
			if kind == scale.Kind && obj.Name == scale.Name {
				res.resMap[unidlingapi.CrossGroupObjectReference{Name: obj.Name, Kind: kind}] = *obj
				return true, obj, nil
			}
		}
...
func TestControllerIgnoresAlreadyScaledObjects(t *testing.T) {
	...
	fakeClient, res := prepFakeClient(t, idledTime, baseScales...)
...
func TestControllerUnidlesProperly(t *testing.T) {
	...
	fakeClient, res := prepFakeClient(t, nowTime.Add(-10*time.Second), baseScales...)
...
func prepareFakeClientForFailureTest(test failureTestInfo) *kfake.Clientset {
	fakeClient := &kfake.Clientset{}
...
func TestControllerPerformsCorrectlyOnFailures(t *testing.T) {
	...
	for _, test := range tests {
		fakeClient := prepareFakeClientForFailureTest(test)
		controller := &UnidlingController{
			scaleNamespacer:     fakeClient.Extensions(),
			endpointsNamespacer: fakeClient.Core(),
		}
