...
type AnnotationFunc func(ref unidlingapi.CrossGroupObjectReference, annotations map[string]string)

func NewScaleAnnotater(scales kextclient.ScaleInterface, dcs deployclient.DeploymentConfigInterface, rcs kclient.ReplicationControllerInterface, changeAnnots AnnotationFunc) *ScaleAnnotater {
...
type ScaleAnnotater struct {
	scales            kextclient.ScaleInterface
	dcs               deployclient.DeploymentConfigInterface
	rcs               kclient.ReplicationControllerInterface
...
func (c *ScaleAnnotater) GetObjectWithScale(ref unidlingapi.CrossGroupObjectReference) (runtime.Object, *kextapi.Scale, error) {
	...
	case ref.Kind == "ReplicationController" && ref.Group == kapi.GroupName:
		...
		obj = rc
	default:
		scale, err = c.scales.Get(ref.Kind, ref.Name)
		if err != nil {
			return nil, nil, err
		}
	}
...
func (c *ScaleAnnotater) UpdateObjectScale(ref unidlingapi.CrossGroupObjectReference, obj runtime.Object, scale *kextapi.Scale) error {
	var err error

	if obj == nil {
		_, err = c.scales.Update(ref.Kind, scale)
		return err
	}
	...
	case *deployapi.DeploymentConfig:
		...
		c.changeAnnotations(ref, typedObj.Annotations)
		typedObj.Spec.Replicas = scale.Spec.Replicas
		_, err = c.dcs.Update(typedObj)
