...
func setIdleAnnotations(serviceName string, annotations map[string]string, scaleRefs []unidlingapi.RecordedScaleReference) error {
...
func RunIdle(f *clientcmd.Factory, out io.Writer, cmd *cobra.Command, filename string, loadByService, dryRun bool) error {
	...
	scales := make(map[unidlingapi.CrossGroupObjectReference]int32, len(byScalable))

	mapper, typer := f.Object(false)

	// actually "idle" the scalables by scaling them down to zero
	for scaleRef, svcName := range byScalable {
		scale, err := delegScaleNamespacer.Scales(namespace).Get(scaleRef.Kind, scaleRef.Name)
		if err != nil {
		...
		if !dryRun {
			scales[scaleRef] = scale.Spec.Replicas
			scale.Spec.Replicas = 0

			if _, err := delegScaleNamespacer.Scales(namespace).Update(scaleRef.Kind, scale); err != nil {
				fmt.Fprintf(out, "Unable to scale %s %s/%s to 0, not marking that scalable as idled...\n", scaleRef.Kind, namespace, scaleRef.Name)
				svcInfo := byService[svcName]
				delete(svcInfo.scalables, scaleRef)
				continue
			}
	...
	for serviceName, info := range byService {
		...
		refsWithScale, err := pairScalesWithIdlables(serviceName, info.obj.Annotations, info.scalables, scales)
		if err != nil {
		...
		if !dryRun {
			...
			if err = setIdleAnnotations(serviceName, info.obj.Annotations, refsWithScale); err != nil {
				fmt.Fprintf(out, "Unable to mark service %s/%s as idled: %v", namespace, serviceName, err)
				continue
			}
